...
int main (int argc, char **argv) {
    ...
    while ((opt = getopt(argc, argv, "PCcLt:p:b:z:qd:o:eX:AM:f:")) != -1) {
        switch (opt) {
            case 'P':
            case 'c':
            case 'C':
    ...
    if (mode == "P") {
         ...
         * Create topic handle.
         */
        auto topic = std::unique_ptr<RdKafka::Topic>(RdKafka::Topic::create(producer.get(), topic_str,
                                                                            tconf, errstr));
        if (!topic) {
         ...
         */
        auto consumer = std::unique_ptr<RdKafka::Consumer>(RdKafka::Consumer::create(conf, errstr));
        if (!consumer) {
         ...
         * Create topic handle.
         */
        auto topic = std::unique_ptr<RdKafka::Topic>(RdKafka::Topic::create(consumer.get(), topic_str,
                                                       tconf, errstr));
        if (!topic) {
            std::cerr << "Failed to create topic: " << errstr << std::endl;
            exit(1);
        ...
        while (run) {
            if (use_ccb) {
                consumer->consume_callback(topic.get(), partition, 1000,
                                           &ex_consume_cb, &use_ccb);
            } else {
                RdKafka::Message *msg = consumer->consume(topic.get(), partition, 1000);
